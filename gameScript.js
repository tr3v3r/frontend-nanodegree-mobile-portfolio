...
let {cycleBevior} = __webpack_require__(8);
let {paceBehavior} = __webpack_require__(9);
...
function createBombSprites(now) {
	if(atacking&&now-lastTimeFired > 200) {
		lastTimeFired = now;
	let bomb = new SpriteSheetArtist(bombSheet,bobmCells)

	player.bomb = new Sprite('bomb',bomb,[bombMoveBehavior,new collideBehavior(explode)]);
	player.bomb.width = 30;
	player.bomb.height = 32;

	player.bomb.top = player.top + 10;
	player.bomb.left = player.left +platformOffset+ 20;
	player.bomb.visible = true;	
	sprites.push(player.bomb);
	}
}
...
module.exports = {
    setSpriteOffset,drawSprites,updateSprites,createBombSprites,explode,createDeath};
...
function processCollision(otherSprites) {
	if(otherSprites.type === "papparazzi") {
      if(!player.dying&&player.visible) {        
            createDeath();
       }		
	}
...
function isNearTheEdgeOfTheScreen()
 {
 	if(player.left < 100) movingLeft = false;
...
drawPlatform,offsetSheet,drawPlayer,runPlayer};
...
let bombMoveBehavior = {
	execute(sprite,now) {
		...
		if(!spriteInView(sprite)&&sprite.type ==="bomb"||player.dying) {
...
let {AnimationTimer} = __webpack_require__(1);
...
runPlayer} = __webpack_require__(5);
	
let {jumpExecute,executeFalling,player} = __webpack_require__(1);
...
class collideBehavior {
    ...
    for(let i = 0;i < sprites.length; i++) {
        ...
        if(otherSprites.type!=="bomb"&&otherSprites.type!=="platform") {
...
function processCollision(sprite,otherSprites,callback) {
    if(otherSprites.type === "papparazzi"&&!otherSprites.exploding) {                              
